...
const translations = {
  ...
  'api_import.privacy': { ko: '토큰은 브라우저 메모리에서만 사용되며 저장하지 않습니다. 토큰은 GitHub API 호출에만 전송되고, signed report URL은 토큰 없이 다운로드합니다. API 버전은 항상 2026-03-10입니다.', en: 'The token is used only in browser memory and is not stored. It is sent only to GitHub API; signed report URLs are downloaded without the token. The API version is always 2026-03-10.' },
